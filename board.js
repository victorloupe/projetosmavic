...
function createCardHTML(p, cardIdx=0){
  ...
  if (revLogs.length > 0) {
    ...
    if (latestLog && latestLog.text) {
      const isClient = Boolean(latestLog.author && latestLog.author.toLowerCase() !== 'mavic' && latestLog.author.toLowerCase() !== 'equipe');
      const isTeam = Boolean(latestLog.author && (latestLog.author.toLowerCase() === 'equipe' || latestLog.author.toLowerCase() === 'mavic'));
      const isAuto = Boolean(latestLog.autoGenerated);

      let noteTitle = 'Última anotação de alteração:';
      let noteIcon = 'bi-journal-text';
      let noteColor = 'var(--text2)';
      let noteBg = 'var(--surface2)';
      let noteBorder = 'var(--border)';

      if (isClient) {
        noteTitle = `Ajuste do Cliente (${escapeHtml(latestLog.author || 'Cliente')}):`;
        noteIcon = 'bi-chat-left-dots-fill';
        noteColor = '#ea580c';
        noteBg = 'rgba(234,88,12,0.08)';
        noteBorder = 'rgba(234,88,12,0.3)';
      } else if (isTeam) {
        noteTitle = 'Minha Alteração:';
        noteIcon = 'bi-pencil-square';
        noteColor = 'var(--accent)';
        noteBg = 'rgba(146,98,58,0.08)';
        noteBorder = 'rgba(146,98,58,0.3)';
      } else if (isAuto) {
        noteTitle = 'Retorno de Etapa:';
        noteIcon = 'bi-arrow-left-right';
        noteColor = 'var(--accent)';
        noteBg = 'rgba(146,98,58,0.06)';
        noteBorder = 'rgba(146,98,58,0.25)';
      }

      const dLog = latestLog.timestamp ? new Date(latestLog.timestamp) : (latestLog.date ? new Date(latestLog.date + 'T12:00:00') : null);
      const dStr = dLog ? dLog.toLocaleDateString('pt-BR') : '';
      const pinsBadgeHtml = (Array.isArray(latestLog.pins) && latestLog.pins.length > 0)
        ? `<div style="margin-top:2px"><span class="badge" style="font-size:9.5px;background:rgba(234,88,12,0.15);color:#ea580c;padding:1px 5px"><i class="bi bi-pin-map-fill"></i> ${latestLog.pins.length} ponto${latestLog.pins.length > 1 ? 's' : ''} na imagem</span></div>`
        : '';

      latestClientRevNoteHtml = `
        <div class="kcard-rev-note" style="background:${noteBg};border:1px solid ${noteBorder};border-radius:8px;padding:7px 9px;margin-top:6px;font-size:11.5px;display:flex;flex-direction:column;gap:3px;cursor:pointer" onclick="openRevisionLogModal(${p.id});event.stopPropagation()" title="Clique para ver todo o histórico de alterações">
          <div style="display:flex;align-items:center;justify-content:space-between;font-weight:700;color:${noteColor};font-size:11px">
            <span><i class="bi ${noteIcon}"></i> ${noteTitle}</span>
  ...
  const hasAdjustment = Boolean(p.pendingClientRevision || (isDevOrAlt && revLogs.length > 0));
  ...
  if (hasAdjustment) {
    ...
    bellTitle = '⚠️ Ajuste Solicitado! Clique para ler o que precisa alterar';
  ...
  const prevColObj = (curColIdx > 0) ? allVisCols[curColIdx - 1] : null;
  ...
  const mobileStageBarHtml = `
    <div class="kcard-mobile-stage-bar">
      ...
      <div style="display:inline-flex;align-items:center;gap:4px;margin-left:auto">
        ${prevColObj ? `
          <button type="button" class="btn-quick-prev-col" onclick="quickMovePrev(event, ${p.id})" title="Voltar para ${escapeHtml(prevColObj.id)} (+1 alteração)">
  ...
  return `<div class="kcard ${dlClass} ${pinnedCards.has(p.id)?'pinned':''}" data-id="${p.id}" draggable="true" onclick="togglePin(event,${p.id})" style="--type-color:${typeColor(p.type)}">
    ...
    <div class="kcard-exp">
      ...
      <div class="cact">
        ...
        ${prevColObj ? `<button class="cbtn" style="margin-left:auto;color:var(--text3)" onclick="movePrev(${p.id});event.stopPropagation()" title="Voltar para ${escapeHtml(prevColObj.id)} (+1 alteração)"><i class="bi bi-arrow-left-circle"></i></button>` : ''}
        ${nextColObj ? `<button class="cbtn" style="${prevColObj ? '' : 'margin-left:auto;'}color:var(--accent)" onclick="moveNext(${p.id});event.stopPropagation()" title="Avançar para ${escapeHtml(nextColObj.id)}"><i class="bi bi-arrow-right-circle"></i></button>` : ''}
      </div>
    </div>
  </div>`;
}
...
let pendingStageReturn = null;

function openStageReturnModal(p, oldCol, newCol, suppressToast = false) {
  pendingStageReturn = { projId: p.id, oldCol, newCol, suppressToast };

  const overlay = document.getElementById('stageReturnModalOverlay');
  if (!overlay) {
    applyColumnChange(p, newCol, suppressToast, true);
    renderBoard();
    scheduleSync();
    return;
  }

  const revCount = (parseInt(p.revisions) || 0) + 1;
  const revLimit = parseInt(p.revisionsLimit !== undefined ? p.revisionsLimit : 2);
  const isOver = revCount > revLimit;
  const extra = Math.max(0, revCount - revLimit);

  const infoEl = document.getElementById('stageReturnModalInfo');
  if (infoEl) {
    infoEl.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
        <div style="font-weight:700;font-size:13.5px;color:var(--text)">${escapeHtml(p.name)}</div>
        <span class="badge" style="background:rgba(146,98,58,0.15);color:var(--accent);font-weight:700;font-size:11px">
          ${revCount}ª Alteração
        </span>
      </div>
      <div style="font-size:12px;color:var(--text2);display:flex;align-items:center;gap:6px;margin-top:2px">
        <span>Retornando de: <strong>${escapeHtml(oldCol)}</strong></span>
        <i class="bi bi-arrow-right" style="color:var(--accent)"></i>
        <span>Para: <strong>${escapeHtml(newCol)}</strong></span>
      </div>
      ${isOver ? `
        <div style="margin-top:4px">
          <span class="badge" style="background:#dd6b20;color:#fff;font-weight:700;font-size:10.5px;padding:2px 6px">
            ⚠️ Limite contratado (${revLimit}) excedido: +${extra} Extra a faturar
          </span>
        </div>
      ` : ''}
    `;
  ...
  const ta = document.getElementById('stageReturnNotes');
  if (ta) {
    ta.value = '';
...
function cancelStageReturnModal() {
  const overlay = document.getElementById('stageReturnModalOverlay');
  if (overlay) overlay.classList.remove('open');
  pendingStageReturn = null;
  renderBoard();
  showToast('Movimentação cancelada', 'info');
}

function confirmStageReturnModal(skipNotes = false) {
  if (!pendingStageReturn) return;
  const { projId, oldCol, newCol, suppressToast } = pendingStageReturn;
  const p = projects.find(x => x.id === projId);
  if (!p) {
    cancelStageReturnModal();
    return;
  }

  let notes = '';
  if (!skipNotes) {
    notes = document.getElementById('stageReturnNotes')?.value?.trim() || '';
  }
  ...
  applyColumnChange(p, newCol, suppressToast, true, notes);
  renderBoard();
  scheduleSync();
}

// Suporte a atalhos no modal de retorno de etapa
document.addEventListener('keydown', (e) => {
  ...
  if (overlay && overlay.classList.contains('open')) {
    if (e.key === 'Escape') {
      ...
      e.preventDefault();
      confirmStageReturnModal(false);
    }
  }
});

function applyColumnChange(p, newCol, suppressToast = false, skipPrompt = false, customNotes = '') {
  if (!p || !newCol || p.column === newCol) return false;
  const oldCol = p.column;

  // Determina dinamicamente a ordem das colunas no fluxo do quadro
  const colsList = (Array.isArray(appColumns) && appColumns.length > 0)
    ? appColumns
    : (typeof INIT_COLS !== 'undefined' ? INIT_COLS : []);

  const findColIdx = (colName) => {
    if (!colName) return -1;
    let i = colsList.findIndex(c => c.id === colName);
    if (i === -1) {
      i = colsList.findIndex(c => (c.id || '').toLowerCase().trim() === colName.toLowerCase().trim());
    }
    return i;
  };

  const oldIdx = findColIdx(oldCol);
  ...
  // É uma etapa anterior se o novo índice for menor que o anterior, ou pelo fallback de palavras-chave
  const isGoingBack = (oldIdx !== -1 && newIdx !== -1)
    ? (newIdx < oldIdx)
    : (isFromAdv && isToDev);

  // Se o projeto está voltando de etapa e ainda não abriu o modal de motivo, abre para perguntar
  if (isGoingBack && !skipPrompt) {
    openStageReturnModal(p, oldCol, newCol, suppressToast);
    return false;
  }
  ...
  if (isHiddenColumn(newCol) || newCol === 'Finalizado') {
    ...
    // 1. Registro no Histórico de Alterações (com anotação da equipe se preenchida)
    if (!Array.isArray(p.revisionLogs)) p.revisionLogs = [];
    const logId = Date.now();
    const hasCustomNotes = Boolean(customNotes && customNotes.trim());
    const logText = hasCustomNotes
      ? customNotes.trim()
      : `Projeto retornou da etapa "${oldCol}" para "${newCol}".`;
    ...
    p.revisionLogs.push({
      ...
      date: today(),
      text: logText,
      author: hasCustomNotes ? 'Equipe' : '',
      autoGenerated: !hasCustomNotes
    });

    // 2. Alerta de Limite Contratado Excedido
    const revLimit = parseInt(p.revisionsLimit !== undefined ? p.revisionsLimit : 2);
    const isOverLimit = p.revisions > revLimit;
    const extraCount = Math.max(0, p.revisions - revLimit);

    let toastMsg = `Projeto retornou para ${escapeHtml(newCol)}: +1 alteração computada (${p.revisions}ª alteração)!`;
    if (isOverLimit) {
      toastMsg = `⚠️ Projeto retornou para ${escapeHtml(newCol)}: ${p.revisions}ª alteração (+${extraCount} Extra além do limite!)`;
    }

    // 3. Ação de Desfazer (Undo) no Toast
    if (!suppressToast) {
      const undoAction = {
        text: 'Desfazer',
        onClick: () => {
          p.column = oldCol;
          p.revisions = Math.max(0, (parseInt(p.revisions) || 0) - 1);
          if (Array.isArray(p.revisionLogs)) {
            p.revisionLogs = p.revisionLogs.filter(l => l.id !== logId);
          }
          renderBoard();
          scheduleSync();
          showToast(`Retorno desfeito! "${escapeHtml(p.name)}" voltou para ${escapeHtml(oldCol)}.`, 'info');
        }
      };
      showToast(toastMsg, 'warning', undoAction);
    }
  } else {
    if (!suppressToast) {
      ...
      <div style="background:${isClient ? 'rgba(234,88,12,0.06)' : 'var(--surface2)'};border:1px solid ${isClient ? 'rgba(234,88,12,0.3)' : 'var(--border)'};border-radius:8px;padding:9px 12px;display:flex;flex-direction:column;gap:5px">
        <div style="display:flex;justify-content:space-between;align-items:center;font-size:11px;color:var(--text3)">
          <div style="display:flex;align-items:center;gap:6px">
            ...
            ${isClient ? `<span class="badge" style="background:rgba(234,88,12,0.15);color:#ea580c;font-size:9.5px;padding:1px 5px;font-weight:700"><i class="bi bi-person-fill"></i> Solicitado por ${escapeHtml(lg.author || 'Cliente')}</span>` : ((lg.author && (lg.author.toLowerCase() === 'equipe' || lg.author.toLowerCase() === 'mavic')) ? `<span class="badge" style="background:rgba(146,98,58,0.15);color:var(--accent);font-size:9.5px;padding:1px 5px;font-weight:700"><i class="bi bi-pencil-fill"></i> Alteração da Equipe</span>` : (lg.autoGenerated ? `<span class="badge" style="background:rgba(146,98,58,0.12);color:var(--accent);font-size:9.5px;padding:1px 5px;font-weight:700"><i class="bi bi-arrow-left-right"></i> Retorno de Etapa</span>` : ''))}
          </div>
          <div style="display:flex;align-items:center;gap:6px">
...
function moveNext(id){
  ...
  applyColumnChange(p, targetCol, true);
  if (typeof triggerHaptic === 'function') triggerHaptic('medium');
  showToast(`Avançado para "${targetCol}"`, 'success');
  renderBoard();scheduleSync();
}

function quickMovePrev(e, id) {
  if (e) {
    e.preventDefault();
    e.stopPropagation();
  }
...
function movePrev(id){
  ...
  const cols = appColumns.filter(c => visibleColumns.includes(c.id));
  const idx = cols.findIndex(c => c.id === p.column);
  if (idx <= 0) return showToast('Já na primeira etapa', 'info');
  const targetCol = cols[idx - 1].id;
